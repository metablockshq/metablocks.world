...
const AuthorBar = ({ author, publishedOn }) => {
  ...
  return (
    <div className="flex flex-column flex-row-m flex-row-l justify-between items-center bt b--black-20 ph3 ph4-l mt2 mt3-ns pb2 pt0 pt1-ns f7 f6-l">
      ...
      <div className="black-80">{str.humanReadableDate(publishedOn)}</div>
    </div>
  );
};

const stackClass = (additional) =>
  `br4 br--top h1 bg-white center ${additional}`;
...
const PostCard = ({ post, leftTopTitle, stacked, containerClass }) => {
  const { title, subTitle, heroImg, tags, publishedOn, author, featured } =
    post.data;
  ...
  return (
    <Link to={post.path} className="">
      {stacked && <Stack />}
      <div
        className={`flex flex-column bg-white br4 postCard ${containerClass}`}
      ...
      >
        ...
        <div
          className={`flex items-center pt2 ph3 ph4-ns ${
            featured && "flex-column"
          }`}
        >
          <div
            className={featured ? "mb2 mb3-ns w-100" : "normalPostFlexBasis"}
          ...
          <div className={`pl3 tl ${featured && "tc"}`}>
            ...
            {subTitle && (
              <div className="mt1 black-80 f5 black-80">{subTitle}</div>
            )}
          </div>
        </div>

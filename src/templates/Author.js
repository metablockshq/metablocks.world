...
const SocialLinks = ({ twitter, youtube, github, medium }) => {
  return (
    <ul>
      {twitter && (
        <Li title={"Twitter"} href={`https://twitter.com/${twitter}`} />
      )}
    ...
    <React.Fragment>
      ...
      <Shell>
        <div className="w-90 w-70-m w-50-l center flex items-center">
          <img src={profilePicture} alt={name} className="w-30 br-100" />
          <div className="pl3">
            <div className="f4 f3-m f2-l b">{name}</div>
            <SocialLinks
              twitter={twitter}
              github={github}
              medium={medium}

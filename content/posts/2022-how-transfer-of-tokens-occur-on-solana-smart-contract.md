...
Now that we have a minted an `spl_token_mint`, it is time to understand about Associated Token Account (ATA) before transferring to an account. 
...
The given code is an example ATA generation using `PublicKey.findProgramAddress` provided by `@solana/web3.js` library on the client side.
